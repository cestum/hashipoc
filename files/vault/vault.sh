...
cp /vagrant/vault/config.hcl /etc/vault/config.hcl
cp /vagrant/vault/supervisor.vault.conf /etc/supervisor/conf.d/vault.conf

supervisorctl reread
supervisorctl update vault
supervisorctl stop vault || true

 # Make sure vault is up and running
# while ! curl $VAULT_ADDR >& /dev/null; do
#     sleep 1
# done
#if dev mode is enabled root token will be enabled in log
# while [ "$TOKEN" == "" ]; do
#     TOKEN=$(grep 'Root Token' /var/log/vault/out | tail -n1 | awk '{print $3}')
#     sleep 1
# done
# echo "Found root token in logs $TOKEN"
...
echo "VAULT_SKIP_VERIFY=$VAULT_SKIP_VERIFY" >> /etc/environment
